...
export default function ScenarioQuestPage() {
  ...
  const handleNext = async () => {
    ...
    if (current + 1 >= scenarios.length) {
      setExtracting(true);
      // Calculate traits from all responses
    ...
    return (
      <div className="max-w-3xl mx-auto flex items-center justify-center min-h-[60vh]">
        <Card className="text-center px-8 py-12">
          ...
          <h2 className="font-display mb-2 text-3xl text-white">Analyzing Your Responses...</h2>
          <p className="text-white/75">Our AI is extracting your personality traits</p>
        </Card>
      </div>
    );
  }
    ...
    return (
      <div className="max-w-3xl mx-auto space-y-6">
        <Card className="text-center">
          ...
          <h2 className="font-display mb-2 text-3xl text-white">Scenario Quest Complete!</h2>
          <p className="mb-6 text-white/78">Here are the traits we detected from your decisions:</p>
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <div className="surface-panel rounded-[1.6rem] p-6">
        <h1 className="font-display mb-1 text-3xl text-white">🎯 Scenario Quest</h1>
        <p className="text-sm text-white/78">Navigate workplace scenarios to reveal your decision-making style</p>
      </div>

      <ProgressBar value={current + 1} max={scenarios.length} label={`Question ${current + 1} of ${scenarios.length}`} />

      <Card>
        <p className="mb-6 text-lg font-medium text-white">{scenario.text}</p>
        <div className="space-y-3">
          {scenario.options.map((option, idx) => (
            <button
              key={idx}
              onClick={() => handleSelect(idx)}
              className={`w-full rounded-xl border px-4 py-4 text-left text-sm transition-all ${
                selectedOption === idx
                  ? 'border-primary-300/60 bg-primary-500/25 text-white'
