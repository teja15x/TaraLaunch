...
exports.detectRoleMentionFromText = detectRoleMentionFromText;
exports.getRoleKnowledgeSummary = getRoleKnowledgeSummary;
exports.buildRoleKnowledgePromptBlock = buildRoleKnowledgePromptBlock;
const careerRoles_1 = require("@/data/careerRoles");
...
const COLLEGE_GUIDANCE_BY_FAMILY = {
    technology: {
...
function resolveRoleTitle(rawRoleTitle) {
    const normalizedInput = normalizeText(rawRoleTitle);
    const exact = careerRoles_1.CAREER_ROLE_OPTIONS.find((role) => normalizeText(role.title) === normalizedInput);
    if (exact)
        return exact.title;
    const closest = careerRoles_1.CAREER_ROLE_OPTIONS.find((role) => {
...
function resolveSalaryReference(roleTitle) {
    const normalized = normalizeText(roleTitle);
    const alias = ROLE_ID_ALIAS_MAP[normalized];
    if (alias) {
        const fromAlias = careers_1.careerDatabase.find((career) => career.id === alias);
        if (fromAlias?.salary_range_inr)
            return fromAlias.salary_range_inr;
    }
    const byTitle = careers_1.careerDatabase.find((career) => {
...
function mergeProfileWithOverride(profile, roleTitle) {
...
function detectRoleMentionFromText(text) {
    if (!text)
        return null;
    const normalizedText = normalizeText(text);
    if (!normalizedText)
        return null;
    const orderedRoles = [...careerRoles_1.CAREER_ROLE_OPTIONS]
        .map((role) => role.title)
        .sort((a, b) => b.length - a.length);
    for (const roleTitle of orderedRoles) {
        const normalizedRoleTitle = normalizeText(roleTitle);
        if (!normalizedRoleTitle || normalizedRoleTitle === 'not sure yet')
            continue;
...
function getRoleKnowledgeSummary(selectedRole) {
...
function buildRoleKnowledgePromptBlock(selectedRole) {
