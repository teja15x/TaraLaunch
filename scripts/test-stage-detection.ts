import { detectStudentStage, deriveCounselingTrack, StudentStage } from '../src/lib/career-agent/stageDetection';

type StageCase = {
  name: string;
  intake: { currentStage?: string; currentSituation?: string };
  expected: StudentStage;
};

type TrackCase = {
  name: string;
  input: { currentStage?: string; currentSituation?: string; detectedStage?: string };
  expected: 'career-counseling' | 'school-exam-support' | 'post-college-employability';
};
