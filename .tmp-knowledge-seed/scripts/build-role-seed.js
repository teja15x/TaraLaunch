...
const careerRoles_1 = require("../src/data/careerRoles");
const MANUAL_EXTRA_ROLES = [
...
function inferFamily(title, hint) {
    const text = `${title} ${hint}`.toLowerCase();
    if (/software|data|ai|ml|cyber|cloud|devops/.test(text))
        return 'technology';
    if (/account|finance|bank|investment|ca|cma|cs/.test(text))
        return 'finance';
    if (/doctor|dentist|nurse|physio|pharma|nutrition|clinical/.test(text))
        return 'healthcare';
    if (/law|judge|judicial|civil services|ias|ips|upsc|public service/.test(text))
        return 'law-governance';
    if (/designer|ux|ui|graphic|animator|fashion|interior/.test(text))
        return 'design-creative';
    if (/product manager|entrepreneur|manager|consultant|analyst/.test(text))
        return 'business-management';
    if (/mechanical|civil engineer|electrical|electronics|robotics|automobile/.test(text))
        return 'engineering-core';
    if (/teacher|professor|lecturer|research/.test(text))
        return 'education-research';
    if (/hotel|chef|event|service/.test(text))
        return 'hospitality-services';
    if (/sports|athlete|coach/.test(text))
        return 'sports-performance';
    if (/journalist|content|creator|youtuber|photographer|videographer|translator/.test(text))
        return 'media-communication';
    if (/pilot|cabin crew|air traffic|merchant navy/.test(text))
        return 'aviation-maritime';
    if (/psycholog|counselor/.test(text))
        return 'psychology-counseling';
    if (/defense|government/.test(text))
        return 'public-sector';
    return 'exploration';
}
function inferStageFit(title, hint) {
    const text = `${title} ${hint}`.toLowerCase();
...
const seeds = careerRoles_1.CAREER_ROLE_OPTIONS
...
const outPath = (0, node_path_1.join)(process.cwd(), 'src', 'data', 'knowledge', 'roles.seed.json');
(0, node_fs_1.writeFileSync)(outPath, `${JSON.stringify(result, null, 2)}\n`, 'utf-8');
